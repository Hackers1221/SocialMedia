...
import { clearRelatedPosts, getPostById, getRelatedPosts, getSavedPost, likePost, updateSavedPost } from "../redux/Slices/post.slice";
...
const DisplayPost = () => {
    ...
    useEffect (() => {
        if (postList?.length) {
            setPostIndex (postList.indexOf (postId)); return;
        }
        if (!postId) return;
    ...
    useEffect(() => {
        const handleKeyPress = (event) => {
            if (event.key === "ArrowRight") setPostIndex((prev) => (prev + 1) % postList.length);
            if (event.key === "ArrowLeft") setPostIndex((prev) => (prev - 1 + postList.length) % postList.length);
        };
