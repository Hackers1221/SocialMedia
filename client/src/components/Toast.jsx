  ...
  return (
    <div
      className={`
        ...
        border-2 transition-all duration-300 ease-in-out transform w-max max-w-[90%] md:w-max
        ${type === "notification" ? "bg-[#F0E68C] border-[#FEBE10]" : type !== 'error' ? 'bg-[#AFEEEE] border-[var(--buttons)]' : 'bg-red-200 border-red-700 text-red-700'}
    ...
    >
      ...
      {type == "notification" && <i className="fa-solid fa-bell text-[#CC551A]"></i>}
      <h2 className='w-max'>{message}</h2>
    </div>
  );
};
